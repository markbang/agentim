...
- `prek install`：按仓库配置安装 `pre-commit` 和 `pre-push` 两类 git hooks
- `prek run --all-files`：运行文件清理和 `cargo fmt`
- `prek run --all-files --hook-stage pre-push`：运行 `cargo clippy --all-targets --all-features -- -D warnings`
- `cargo test`：单元测试和集成测试
- `review_bridge`：重点验证 webhook、session、reply target、鉴权和持久化
- `autoresearch.sh`：输出结构化 acceptance metrics

这个仓库当前使用原生 [prek.toml](prek.toml) 配置，并在 CI 里通过 `j178/prek-action` 执行同一套检查。
